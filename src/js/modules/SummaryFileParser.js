...
export class SummaryFileParser {
    ...
    parseSummaryFile(content) {
        try {
            ...
            console.log(content.substring(0, 1000)); // First 1000 characters
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                // Debug: Log every line for analysis
                if (line.length > 0) {
                ...
                switch (currentSection) {
                    ...
                    case this.sections.TEST_RESULTS:
                        ...
                        if (testResult) {
                            ...
                            console.log(`Added test result: ${testResult.description} = ${testResult.result} (Soft Bin ${testResult.softBin}, Hard Bin ${testResult.hardBin})`);
                        }
                        break;
                }
            }
            
            console.log(`Total test results parsed: ${testResults.length}`);
            console.log('Test results:', testResults);
            
            // Calculate maximum Soft Bin number from parsed results
            const maxSoftBin = testResults.length > 0 ? Math.max(...testResults.map(t => t.softBin)) : 1;
            console.log(`Maximum Soft Bin number found: ${maxSoftBin}`);
    ...
    parseTestResultLine(line) {
        ...
        // Pattern 0: Actual .lotSumTXT format
        // Format: "   1    1    PASS PASS                                              1854( 88.0)    449     498     424     483   "
        ...
        if (actualFormatMatch) {
            ...
            console.log(`Parsed actual format: ${description} = ${result} (Soft Bin ${softBin}, Hard Bin ${hardBin})`);
        ...
        // Pattern 1: Tab-separated format with test names
        // Format: "OSN_TEST	FAIL	2	0	1	1	0"
        const tabSeparatedMatch = trimmedLine.match(/^([A-Z0-9_]+)\s+(PASS|FAIL)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)/);
        if (tabSeparatedMatch) {
            const [, description, result, total, site1, site2, site3, site4] = tabSeparatedMatch;
            
            // Determine Soft Bin and Hard Bin based on test name
            const softBin = this.getSoftBinFromTestName(description);
            const hardBin = this.getHardBinFromTestName(description);
            
            console.log(`Parsed tab-separated format: ${description} = ${result} (Soft Bin ${softBin}, Hard Bin ${hardBin})`);
        ...
        // Pattern 2: Standard format "1 1 FAIL Test1 100 10.0 50 30 10 10"
        ...
        // Pattern 3: Simple format "Test1: PASS 950 FAIL 50 Total 1000"
        ...
        // Pattern 4: Yield format "Yield: 94.0%"
        ...
        // Pattern 5: Generic test result with numbers "Test1 100 50 30 10 10"
        ...
        // Pattern 6: Test with PASS/FAIL indicator "Test1 PASS 100"
        ...
        // Pattern 7: Any line with numbers that might be test data
        ...
        if (trimmedLine.match(/^\w+/) && !trimmedLine.includes('*') && !trimmedLine.includes('Total') && !trimmedLine.includes('Yield')) {
            console.log(`Unmatched line that might be test data: "${trimmedLine}"`);
        }
    ...
    parseHardbinSection(lines, startIndex) {
        const hardbinData = { bins: {} };
        
        for (let i = startIndex; i < lines.length; i++) {
            const line = lines[i];
            
            if (line.includes('* HARDBIN')) {
                console.log(`Found HARDBIN section at line ${i + 1}: "${line}"`);
                
                // Bin numbers are on the same line as * HARDBIN
                const binHeaders = line.match(/\d+/g);
                
                // Bin counts are on the next line
                if (i + 1 < lines.length) {
                    const countLine = lines[i + 1].trim();
                    const counts = countLine.match(/\d+/g);
                    
                    console.log(`HARDBIN header line: "${line}"`, binHeaders);
                    console.log(`HARDBIN count line: "${countLine}"`, counts);

                    if (binHeaders && counts && binHeaders.length === counts.length) {
                        for (let j = 0; j < binHeaders.length; j++) {
                            const binNumber = parseInt(binHeaders[j], 10);
                            const count = parseInt(counts[j], 10);
                            if (!isNaN(binNumber) && !isNaN(count)) {
                                hardbinData.bins[binNumber] = count;
                            }
                        }
                        console.log('Successfully parsed HARDBIN data:', hardbinData);
                    } else {
                        console.warn('Mismatch between HARDBIN headers and counts or no data found.');
                    }
                }
                // Once we've processed the HARDBIN section, we can stop.
                break; 
            }
        }
        
        console.log(`Final HARDBIN data object:`, hardbinData);
        return hardbinData;
    }

    /**
     * Get Soft Bin number from test name
     * @param {string} testName - Test name
     * @returns {number} Soft Bin number
     */
    getSoftBinFromTestName(testName) {
        const softBinMapping = {
            ...
            'FUNCTION_VIH_MIN': 7,
            'FUNCTION_VIH_MAX': 8,
            'FUNCTION_VOH1_1_MIN': 9,
            'FUNCTION_VOH1_1_MAX': 10,
            'FUNCTION_VOH1_2_MIN': 11,
            'FUNCTION_VOH1_2_MAX': 12,
            'FUNCTION_VOH2_1_MIN': 13,
            'FUNCTION_VOH2_1_MAX': 14,
            'FUNCTION_VOH2_2_MIN': 15,
            'FUNCTION_VOH2_2_MAX': 16,
            'FUNCTION_VOH3_MIN': 17,
            'FUNCTION_VOH3_MAX': 18,
            'FUNCTION_VOH4_MIN': 19,
            'FUNCTION_VOH4_MAX': 20,
            'FUNCTION_VOH5_MIN': 21,
            'FUNCTION_VOH5_MAX': 22,
            'FUNCTION_VOL1_1_MIN': 23,
            'FUNCTION_VOL1_1_MAX': 24,
            'FUNCTION_VOL1_2_MIN': 25,
            'FUNCTION_VOL1_2_MAX': 26,
            'FUNCTION_VOL2_1_MIN': 27,
            'FUNCTION_VOL2_1_MAX': 28,
            'FUNCTION_VOL2_2_MIN': 29,
            'FUNCTION_VOL2_2_MAX': 30,
            'FUNCTION_VOL3_MIN': 31,
            'FUNCTION_VOL3_MAX': 32,
            'FUNCTION_VOL4_MIN': 33,
            'FUNCTION_VOL4_MAX': 34,
            'FUNCTION_VOL5_MIN': 35,
            'FUNCTION_VOL5_MAX': 36,
            'FT_M5_SSA_MIN': 53,
            'FT_M5_OCC_MIN': 54,
            'FT_M5_SSA_MAX': 55,
            'FT_M5_OCC_MAX': 56,
            'FT_USB_SSA_MIN': 76,
            'FT_USB_SSA_MAX': 77,
            'FT_USB_FS_BIST_MIN': 78,
            'FT_USB_FS_BIST_MAX': 79,
            'FT_USB_HS_BIST_MIN': 80,
            'FT_USB_HS_BIST_MAX': 81,
            'FT_USB_LS_BIST_MIN': 82,
            'FT_USB_LS_BIST_MAX': 83,
            'FT_USB_HSBIST_LOWSWING_MIN': 88,
            'FT_USB_HSBIST_LOWSWING_MAX': 89,
            'FT_ADC150_MIN_INL_DLE': 105,
            'FT_ADC150_MAX_INL_DLE': 106,
            'FT_ADC150_MIN_AC': 107,
            'FT_ADC150_MAX_AC': 108,
            'FT_ADC150_MIN_SNDR_1': 109,
            'FT_ADC150_MIN_SNDR_2': 110,
            'FT_ADC150_MIN_SNDR_3': 111,
            'FT_ADC150_MIN_SNDR_4': 112,
            'FT_ADC150_MIN_SNDR_5': 113,
            'FT_ADC150_MIN_SNDR_6': 114,
            'FT_ADC150_MIN_SNDR_7': 115,
            'FT_SOC_STBY_TEST': 120,
            'FT_DEVICE_ID_TEST': 123,
            'OVERRIDE_CHECK': 160
        };
        
        return softBinMapping[testName] || 1; // Default to 1 if not found
    }

    /**
     * Get Hard Bin number from test name
     * @param {string} testName - Test name
     * @returns {number} Hard Bin number
     */
    getHardBinFromTestName(testName) {
        const hardBinMapping = {
            'OSN_TEST': 2,
            'OSP_TEST': 2, 
            'PWR_SHORT_TEST': 2,
            'FUNCTION_VIL_MIN': 3,
            ...
            'FUNCTION_VIH_MIN': 3,
            'FUNCTION_VIH_MAX': 3,
            'FUNCTION_VOH1_1_MIN': 3,
            'FUNCTION_VOH1_1_MAX': 3,
            'FUNCTION_VOH1_2_MIN': 3,
            'FUNCTION_VOH1_2_MAX': 3,
            'FUNCTION_VOH2_1_MIN': 3,
            'FUNCTION_VOH2_1_MAX': 3,
            'FUNCTION_VOH2_2_MIN': 3,
            'FUNCTION_VOH2_2_MAX': 3,
            'FUNCTION_VOH3_MIN': 3,
            'FUNCTION_VOH3_MAX': 3,
            'FUNCTION_VOH4_MIN': 3,
            'FUNCTION_VOH4_MAX': 3,
            'FUNCTION_VOH5_MIN': 3,
            'FUNCTION_VOH5_MAX': 3,
            'FUNCTION_VOL1_1_MIN': 3,
            'FUNCTION_VOL1_1_MAX': 3,
            'FUNCTION_VOL1_2_MIN': 3,
            'FUNCTION_VOL1_2_MAX': 3,
            'FUNCTION_VOL2_1_MIN': 3,
            'FUNCTION_VOL2_1_MAX': 3,
            'FUNCTION_VOL2_2_MIN': 3,
            'FUNCTION_VOL2_2_MAX': 3,
            'FUNCTION_VOL3_MIN': 3,
            'FUNCTION_VOL3_MAX': 3,
            'FUNCTION_VOL4_MIN': 3,
            'FUNCTION_VOL4_MAX': 3,
            ...
            'FT_USB_SSA_MIN': 4,
            'FT_USB_SSA_MAX': 4,
            'FT_USB_FS_BIST_MIN': 4,
            'FT_USB_FS_BIST_MAX': 4,
            'FT_USB_HS_BIST_MIN': 4,
            'FT_USB_HS_BIST_MAX': 4,
            'FT_USB_LS_BIST_MIN': 4,
            'FT_USB_LS_BIST_MAX': 4,
            'FT_USB_HSBIST_LOWSWING_MIN': 4,
            'FT_USB_HSBIST_LOWSWING_MAX': 4,
            'FT_ADC150_MIN_INL_DLE': 4,
            'FT_ADC150_MAX_INL_DLE': 4,
            'FT_ADC150_MIN_AC': 4,
            'FT_ADC150_MAX_AC': 4,
            'FT_ADC150_MIN_SNDR_1': 4,
            'FT_ADC150_MIN_SNDR_2': 4,
            'FT_ADC150_MIN_SNDR_3': 4,
            'FT_ADC150_MIN_SNDR_4': 4,
            'FT_ADC150_MIN_SNDR_5': 4,
            'FT_ADC150_MIN_SNDR_6': 4,
            'FT_ADC150_MIN_SNDR_7': 4,
            'FT_SOC_STBY_TEST': 6,
            'FT_DEVICE_ID_TEST': 5,
            'OVERRIDE_CHECK': 6
        };
        
        return hardBinMapping[testName] || 1; // Default to 1 if not found
    }
}

if (typeof window !== 'undefined') window.SummaryFileParser = SummaryFileParser; 